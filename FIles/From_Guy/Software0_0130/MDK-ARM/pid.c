...
/* --------------------------------------------------------------------------
 Control word calculation.  The function returns the required duty cycle
...
unsigned int   result[2];
	...
	for(cushion = 0; cushion < 2; cushion++) { 
		...
		// -----------------------
		i1 = pid_error[cushion] * pid_i_contst[cushion]; // The new value to be added to Integral
...
//  pid_integral[cushion] += i1; // adding to the new value
		...
		if (calc_num == NUM_OF_LOOPS_FOR_CALCULATING_D_PART_OF_PID) {
			...
			pid_error_mx[cushion] = pid_error[cushion]; // Store error as was N loops before for caalculating the Derivative part of PID:
		}
	}
