...
/* --------------------------------------------------------------------------
 Main function to be called for executing command that was requested by the
...
{	
	switch (from_som_message_state.opcode_type) {
		case LED_BUZZER_OPCODE: // a command from SOM to turn on or off the LEDs or the Buzzer
			...
			break;
		case HEAT_AND_AGITATION_OPCODE: // a Technician command to turn on or off the heating elements or the aggitation motor.
			serve_agitation_and_heater_opcode();
			break;
		case TECHNICIAN_MODE_OPCODE: // A command to enter into technician mode.
			serve_technician_opcode();
			break;
		case START_STOP_THAWING_OPCODE: // A command from SOM to start or stop thawing.
...
/* --------------------------------------------------------------------------
 Serving the command from SOM for starting or for stopping thawing.
-------------------------------------------------------------------------- */
void serve_start_stop_thawing_opcode(void)
{
	if (from_som_message_state.opcode_sub_type == START_THAWING_SUB_CODE) { // if command to starat thawing
...
/* --------------------------------------------------------------------------
 Serving the command from SOM for entering or exiting technician mode.
...
/* --------------------------------------------------------------------------
 Serving the command from SOM for turning on or off a LED color.
 This function is to be called after command from SOM was decoded.
-------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
 Serving the command from SOM for turning on or off the aggitation motor,
